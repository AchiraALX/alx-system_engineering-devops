...
if __name__ == "__main__":
    ...
    with urllib.request.urlopen("http://jsonplaceholder.typicode.com/users/{}".format(eid)) as response:
    ...
    total_tasks = 0
    done_tasks = []
    with urllib.request.urlopen("http://jsonplaceholder.typicode.com/todos") as response:
        data = json.loads(response.read().decode())
        for task in data:
    ...
    # Output results
    print("Employee {} is done with tasks({}/{}):".format(name, len(done_tasks), total_tasks))
